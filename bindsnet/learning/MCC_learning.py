...
class PostPre(MCC_LearningRule):
  ...
  def __init__(
    ...
    if self.average_update > 0:
      self.average_buffer_pre = torch.zeros(
        self.average_update, *self.feature_value.shape, device=self.feature_value.device
      )
  ...
  def _connection_update(self, **kwargs) -> None:
    ...
    if self.nu[0]:
      ...
      if self.average_update > 0:
        self.average_buffer_pre[self.average_buffer_index_pre] = (
          self.reduction(torch.bmm(source_s, target_x), dim=0)
        )

        self.average_buffer_index_pre = (self.average_buffer_index_pre + 1) % self.average_update

        if self.continues_update:
          self.feature_value -= torch.mean(self.average_buffer_pre, dim=0) * self.connection.dt
        elif self.average_buffer_index_pre == 0:
          self.feature_value -= torch.mean(self.average_buffer_pre, dim=0) * self.connection.dt
      else:
        self.feature_value -= self.reduction(torch.bmm(source_s, target_x), dim=0) * self.connection.dt
    ...
    if self.nu[1]:
      ...
      if self.average_update > 0:
        ...
        self.average_buffer_index_post = (self.average_buffer_index_post + 1) % self.average_update

        if self.continues_update:
          self.feature_value += torch.mean(self.average_buffer_post, dim=0) * self.connection.dt
        elif self.average_buffer_index_post == 0:
          self.feature_value += torch.mean(self.average_buffer_post, dim=0) * self.connection.dt
      else:
        self.feature_value += self.reduction(torch.bmm(source_x, target_s), dim=0) * self.connection.dt
...
class MSTDP(MCC_LearningRule):
  ...
  def _connection_update(self, **kwargs) -> None:
    ...
    if self.average_update > 0:
      self.average_buffer[self.average_buffer_index] = self.reduction(update, dim=0)
      self.average_buffer_index = (self.average_buffer_index + 1) % self.average_update

      if self.continues_update:
        self.feature_value += self.nu[0] * torch.mean(self.average_buffer, dim=0)
...
class MSTDPET(MCC_LearningRule):
  ...
  def __init__(
    ...
    if not hasattr(self, "p_minus"):
      self.p_minus = torch.zeros((self.target.n), device=self.feature_value.device)
