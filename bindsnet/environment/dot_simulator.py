...
class DotSimulator():
    ...
    def __init__(self, t: int, **kwargs) -> None:
        ...
        self.fileCnt = 0
        self.filename = path + '/' + self.filename[:-5] + '_' + str(self.fileCnt) + self.filename[-4:]
    ...
    def step(self, action):
        ...
        if(random.uniform(0,1) <= self.randr):
            self.dotDir = random.randint(self.minch, self.choices-1)    # five possible options
    ...
    def movePoint(self, d: Dot, dotDir: int = -1):
        ...
        for t in range(self.decay):
            self.obs[d.row[t], d.col[t]] = min(self.obs[d.row[t], 
                                               d.col[t]] + 1 - t/self.decay, 1)
    ...
    def compute_reward(self):
        ...
        if(self.bullseye != 0 and 
           self.dots[0].row[0] == self.netDot.row[0] and
        ...
        # Euclidean distance
        if(self.fit_func == 0):
            reward = -np.hypot(self.dots[0].row[0] - self.netDot.row[0], 
                               self.dots[0].col[0] - self.netDot.col[0])
        
        # Displacement tensor
        elif(self.fit_func == 1):
            reward = torch.Tensor([self.dots[0].row[0] - self.netDot.row[0],
                                   self.dots[0].col[0] - self.netDot.col[0]])
        
        # Range rings; default range ring size = 2
        elif(self.fit_func == 2):
            reward = -np.hypot(self.dots[0].row[0] - self.netDot.row[0], 
                               self.dots[0].col[0] - self.netDot.col[0])//self.ring_size
