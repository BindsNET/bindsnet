    ...
    def compute_decays(self, dt) -> None:
        ...
        if self.traces:
            self.trace_decay = torch.exp(
                torch.div(torch.mul(self.dt, -1.0), self.tc_trace)
            )
            # self.trace_decay = torch.exp(
...
class McCullochPitts(Nodes):
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        self.s = torch.where(
            self.v >= self.thresh, torch.zeros(self.v.shape), torch.ones(self.v.shape)
        )
...
class IFNodes(Nodes):
    ...
    def set_batch_size(self, batch_size) -> None:
        ...
        super().set_batch_size(batch_size=batch_size)
        self.v = torch.mul(
            self.reset, torch.ones(batch_size, *self.shape, device=self.v.device)
        )
...
class LIFNodes(Nodes):
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        self.v = torch.add(
            torch.mul(self.decay, torch.sub(self.v, self.rest)), self.rest
        )
    ...
    def set_batch_size(self, batch_size) -> None:
        ...
        super().set_batch_size(batch_size=batch_size)
        self.v = torch.mul(
            self.rest, torch.ones(batch_size, *self.shape, device=self.v.device)
        )
        self.refrac_count = torch.zeros_like(self.v, device=self.refrac_count.device)
...
class AdaptiveLIFNodes(Nodes):
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        self.s = torch.where(
            self.v >= torch.add(self.thresh, self.theta),
            torch.zeros(self.v.shape),
            torch.ones(self.v.shape),
        )
        ...
        self.v.masked_fill_(self.s, self.reset)
        if self.learning:
            self.theta = torch.add(
                self.theta, torch.mul(self.theta_plus, self.s.float().sum(0))
            )
    ...
    def compute_decays(self, dt) -> None:
        ...
        self.theta_decay = torch.exp(
            torch.div(torch.mul(-1.0, self.dt), self.tc_theta_decay)
        )
        # self.theta_decay = torch.exp(
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        self.v = torch.add(
            ...
            torch.mul(
                ...
                0.5,
                torch.add(
                    torch.mul(0.04, torch.pow(self.v, 2)),
                    torch.mul(5, self.v),
                    140,
                    torch.mul(-1.0, self.u),
                    x,
                ),
            ),
        )
        ...
        self.u = torch.add(
            self.u,
            torch.mul(self.dt, self.a, torch.sub(torch.mul(self.b, self.v), self.u)),
        )
...
class CSRMNodes(Nodes):
    ...
    def AlphaKernel(self, dt):
        ...
        kernelVec = torch.mul(
            torch.div(1.0, torch.pow(self.tau, 2)),
            t,
            torch.exp(torch.div(torch.mul(-1.0, t), self.tau)),
        )
    ...
    def AlphaKernelSLAYER(self, dt):
        ...
        kernelVec = torch.mul(
            torch.div(1.0, self.tau),
            t,
            torch.exp(torch.sub(1.0, torch.div(t, self.tau))),
        )
        return torch.flip(kernelVec, [0])
    ...
    def LaplacianKernel(self, dt):
        ...
        kernelVec = torch.mul(
            torch.div(1.0, torch.mul(self.tau, 2)),
            torch.exp(torch.mul(-1, torch.abs(torch.div(t, self.tau)))),
        )
        return torch.flip(kernelVec, [0])
    ...
    def ExponentialKernel(self, dt):
        ...
        kernelVec = torch.mul(
            torch.div(1, self.tau), torch.exp(torch.div(torch.mul(-1, t), self.tau))
        )
        return torch.flip(kernelVec, [0])
    ...
    def TriangularKernel(self, dt):
        ...
        kernelVec = torch.mul(
            torch.div(1, self.tau), torch.sub(1, torch.div(t, self.tau))
        )
        return torch.flip(kernelVec, [0])
    ...
    def EtaKernel(self, dt):
        ...
        kernelVec = torch.mul(
            -1 * self.reset_const, torch.exp(torch.div(torch.mul(-1, t), self.tau))
        )
        return torch.flip(kernelVec, [0])
...
class SRM0Nodes(Nodes):
    ...
    def forward(self, x: torch.Tensor) -> None:
        ...
        self.rho = torch.mul(
            self.rho_0,
            torch.exp(torch.div(torch.sub(self.v, self.thresh), self.d_thresh)),
        )
