...
parser.add_argument("--randr",      type=float, default=.15)
...
def runSimulator(net, env, spikes, episodes, gran=100, rfname='', pfname=''):
    ...
    for ep in range(episodes):
        ...
        spike_record = torch.zeros((steps, int(gran / dt), env.action_space.n), device=DEVICE)
        #perf_ax = None
        ...
        while not done:
            ...
            probabilities = torch.softmax(torch.sum(spike_record[step-1%steps], dim=0), dim=0)
            ...
            if step % 10 == 0:
                print(f"Iteration: {step} (Time: {time.time() - clock:.4f}); reward: {reward}")
                clock = time.time()
...
def main():
    ...
    for layer in set(network.layers):
        spikes[layer] = Monitor(
            network.layers[layer], state_vars=["s"], time=int(granularity / dt), device=DEVICE)
        network.add_monitor(spikes[layer], name=layer)
    
    
    
    # Load the Dot Simultation environment.
    environment = DotSimulator(steps, decay=decay, herrs=herrs, diag=diag, randr=randr,
                               write=write, mute=mute, bound_hand=boundh, fit_func=fit_func, 
                               allow_stay=allow_stay, pandas=pandas, fpath=OUT_FILE_PATH)
    environment.reset()
    ...
    runSimulator(network, environment, spikes, episodes=trn_eps, 
                 gran=granularity, rfname=rewFile, pfname=perfFile)
    ...
    runSimulator(network, environment, spikes, episodes=tst_eps, 
                 gran=granularity, rfname=rewFile, pfname=perfFile)
