...
for i in range(n_train):
    ...
    if i % update_interval == 0 and i > 0:
        ...
        accuracy["all"].append(
            100
            * torch.sum(labels[i - update_interval : i].long() == all_activity_pred)
            / update_interval
        )
    ...
    if plot:
        if gpu:
            image = (
                pipeline.obs.view(3, 32, 32).cpu().numpy().transpose(1, 2, 0)
                / intensity
            )
            image /= image.max()
            inpt = (
            ...
            weights = (
                network.connections[("X", "Ae")]
                .w.view(3, 32, 32, n_neurons)
                .cpu()
                .numpy()
            )
        else:
            ...
            image /= image.max()
            inpt = 255 - pipeline.encoded["X"].view(time, 3 * 32 * 32).sum(0).view(
                3, 32, 32
            ).sum(0)
            weights = (
                network.connections[("X", "Ae")].w.view(3, 32, 32, n_neurons).numpy()
            )
        ...
        if i == 0:
            ...
            inpt_axes, inpt_ims = plot_input(
                image, inpt, label=labels[i], axes=inpt_axes, ims=inpt_ims
            )
