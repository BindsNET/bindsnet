...
middle = LIFNodes(n=1000, refrac=0, traces=True, thresh=-52.0, rest=-65.0)  # Hidden layer
...
# Input -> hidden.
inpt_middle = Connection(source=layers['X'], target=layers['M'],
                         w=torch.transpose(dqn_network.fc1.weight, 0, 1) * layer1scale)
# hidden -> readout.
middle_out = Connection(source=layers['M'], target=layers['R'],
