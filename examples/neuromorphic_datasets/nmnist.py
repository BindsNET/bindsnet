...
parser.add_argument("--time", type=int, default=92)
...
for i, (sparse_spike_tensor, label) in enumerate(train_dataloader):
    if i > n_train:
        break

    spike_tensor = sparse_spike_tensor.to_dense()
    ...
    if gpu:
        inputs = {"X": spike_tensor.cuda().permute([1,0,2,3,4])}
    else:
        inputs = {"X": spike_tensor.permute([1,0,2,3,4])}
...
# Load MNIST data.
test_dataset = tonic.datasets.NMNIST(save_to='./data',
                                train=False,
                                transform=transform)
...
for step, (sparse_spike_tensor, label) in enumerate(test_dataloader):
    if step > n_test:
        break
