...
ow = 2 * torch.rand(n, 10)
oconn = Connection(exc, output, w=ow, update_rule=m_stdp_et, nu=0.1, wmin=0, wmax=5)
...
for i in range(i):
	...
	out_reward[out_reward < 0] = 0
	out_reward[out_reward > 0] = 0.1
	out_rewards.append(out_reward.sum())
	
	reward = target_rates - avg_rates
	reward *= out_reward @ oconn.w.t()
	reward[reward < 0] = 0
	reward[reward > 0] = 0.1
	...
	if plot:
		if i == 0:
			...
			lines = []
			for perf, label in zip(np.array(perfs).T, ['all fired exc', 'max fired exc', 'max fired out']):
