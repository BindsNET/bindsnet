...
class TestMonitor:
	...
	_if = IFNodes(25); network.add_layer(_if, name='Y')
	conn = Connection(inpt, _if, w=torch.rand(inpt.n, _if.n)); network.add_connection(conn, source='X', target='Y')
	
	inpt_mon = Monitor(inpt, state_vars=['s']); network.add_monitor(inpt_mon, name='X')
	_if_mon = Monitor(_if, state_vars=['s', 'v']); network.add_monitor(_if_mon, name='Y')
	...
	inpt_mon = Monitor(inpt, state_vars=['s'], time=500); network.add_monitor(inpt_mon, name='X')
	_if_mon = Monitor(_if, state_vars=['s', 'v'], time=500); network.add_monitor(_if_mon, name='Y')
...
class TestNetworkMonitor:
	...
	_if = IFNodes(75); network.add_layer(_if, name='Y')
	conn = Connection(inpt, _if, w=torch.rand(inpt.n, _if.n)); network.add_connection(conn, source='X', target='Y')
	
	mon = NetworkMonitor(network, state_vars=['s', 'v', 'w']); network.add_monitor(mon, name='monitor')
	...
	mon = NetworkMonitor(network, state_vars=['s', 'v', 'w'], time=50); network.add_monitor(mon, name='monitor')
