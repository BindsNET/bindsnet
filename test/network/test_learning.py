...
class TestLearningRules:
    ...
    def test_hebbian(self):
        ...
        network.add_layer(
            LIFNodes(n=100, traces=True), name='output'
        )
        ...
        network = Network(dt=1.0)
        network.add_layer(
            Input(shape=[1, 1, 10, 10], traces=True), name='input'
        )
        network.add_layer(
            LIFNodes(shape=[1, 32, 8, 8], traces=True), name='output'
        )
        network.add_connection(
            Conv2dConnection(
                source=network.layers['input'],
    ...
    def test_mstdp(self):
        ...
        network = Network(dt=1.0)
        network.add_layer(
            Input(n=100), name='input'
        )
        network.add_layer(
            LIFNodes(n=100), name='output'
        )
        network.add_connection(
            Connection(
                source=network.layers['input'],
        ...
        network = Network(dt=1.0)
        network.add_layer(
            Input(shape=[1, 1, 10, 10]), name='input'
        )
        network.add_layer(
            LIFNodes(shape=[1, 32, 8, 8]), name='output'
        )
        network.add_connection(
            Conv2dConnection(
                source=network.layers['input'],
